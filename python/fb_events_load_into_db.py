...
fb_access_token = "EAACEdEose0cBABmMYDuySFAEEGS6r1FMMAtxhQjswSrtZCig7Dp2dbjyVbOJ1ZBsdhiNKL8TLZAYoltZBRw6MpJRYnYclLxTXW3Uq4kXiorRebo9XTl3odmKJsX1fjZBBkaD4Cz4yxku3ARclm1EXT02wC77LbjzCRJdbv4vNYwVYOMw0y9xuastoyLlBsE4506rI5RHnVdt1WLpUgzEzIwaVSi4ZCut8ZD"

LOGIN_URL = "https://www.facebook.com/login.php?"
URL = "https://www.facebook.com/search/events/?q=UNSW%20Society"
#URL = "https://www.facebook.com/search/events/?q=UNSW%20Society&filters_rp_events_date=%7B%22name%22%3A%22filter_events_date%22%2C%22args%22%3A%222018-05-21%22%7D"
#URL="https://www.facebook.com/search/events/?q=UNSWArtsandSocialSciences"
...
def main():
    ...
    # 1st this
    check_for_fb_UNSWSocietyAbout_0(LOGIN_URL, USERNAME, PASSWORD)
    # then do this
    check_for_fb_UNSWSocietyEvents(LOGIN_URL, USERNAME, PASSWORD)
    # at the end do this
    check_for_fb_UNSWSocietyEvents_1(LOGIN_URL, USERNAME, PASSWORD)
    # Last check for NEW UNSWSociety
    check_for_fb_UNSWSocietyAbout_1(LOGIN_URL, USERNAME, PASSWORD)
...
def search_fb(UNSWSocietyName,URL,LOGIN_URL,USERNAME,PASSWORD):
    ...
    # Perform login
    result_login = session_requests.post(LOGIN_URL, data = payload, headers = dict(referer = LOGIN_URL))

    # Scrape url1
    result_redarect = session_requests.get(URL, headers = dict(referer = URL))

    print(str(result_login.status_code) + "["+str(result_redarect.status_code) + "]")

    input_str1 = result_redarect.text
...
def get_fb_graph_api_data(page_id):
    ...
    fb_graph_url = api_endpoint + page_id + "?fields=about,fan_count,website,cover,link,username,emails,founded&access_token=" + fb_access_token
...
def check_for_fb_UNSWSocietyEvents_1(LOGIN_URL, USERNAME, PASSWORD):
    conn = sqlite3.connect(database_location)
    c = conn.cursor()
    i=0
    for row in c.execute('''SELECT distinct username
                            ...
                            ''').fetchall():
        sid = str(row[0])
        #URL = "https://www.facebook.com/search/str/"+sid+"/keywords_events"
        URL = "https://www.facebook.com/search/top/?q="+sid+"%20events"
        i = i + 1
        print(str(i) + ":[" + sid + "]: "+URL)
...
def check_for_fb_UNSWSocietyAbout_0(LOGIN_URL, USERNAME, PASSWORD):
    conn = sqlite3.connect(database_location)
    c = conn.cursor()
    for row in c.execute('''SELECT distinct username
                            FROM UNSWSocietyAbout
        ...
        sid = str(row[0])
        # if UNSWSocietyName != "":
        print("get_fb_graph_api_data: " + str(sid))
        get_fb_graph_api_data(sid)
    conn.commit()
    conn.close()

def check_for_fb_UNSWSocietyAbout_1(LOGIN_URL, USERNAME, PASSWORD):
    conn = sqlite3.connect(database_location)
    c = conn.cursor()
    for row in c.execute('''select distinct username  FROM UNSWSocietyEvents
                            where username not in
                            (
                                select username 
                                from UNSWSocietyAbout
                            )
                            and username like '%unsw%'
                            ''').fetchall():
        sid = str(row[0])
        # if UNSWSocietyName != "":
        print("get_fb_graph_api_data: " + str(sid))
